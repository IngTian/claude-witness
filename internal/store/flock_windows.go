...
// The lock spans the whole file (offset 0, length 0xFFFFFFFFFFFFFFFF) and is in
// any case released when the handle closes.
func flockFile(f *os.File) (unlock func(), ok bool) {
	...
	return func() {
		windows.UnlockFileEx(h, 0, 0xFFFFFFFF, 0xFFFFFFFF, ol)
		f.Close()
	}, true
}
