...
func parityWorker(s *store.Store, m *parityMiner) *Worker {
	return &Worker{
		...
		Lenses: []*lens.Lens{
			{Name: store.LensDefault, Global: true, Extract: "extract-default", Review: "review-default", Dimensions: []string{"thinking"}},
			{Name: parityLens, Global: false, Extract: "extract-parity", Review: "review-parity", Dimensions: []string{"thinking"}},
		},
		Config: store.Config{},
		Run:    m.run,
	}
